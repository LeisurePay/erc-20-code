...
contract("MRK", (accounts) => {
  ...
  it("transfers: should be greater than zero", async () => {
    await expectRevert(
      instance.transfer(alice, 0, { from: admin }),
      "Transfer",
      { from: admin, to: alice, value: 0 }
    );
  });
  ...
  it("approvals: msg.sender approves alice of 100 & withdraws 50 & 60 (2nd tx should fail)", async () => {
    ...
    // onto next.
    await expectRevert(
      instance.transferFrom(admin, bob, 60, { from: alice }),
  ...
  it("approvals: allow alice 100 to withdraw from admin. Withdraw 60 and then approve 0 & attempt transfer.", async () => {
    ...
    await expectRevert(
      instance.transferFrom.call(admin, bob, 10, { from: alice }),
      "ERC20: transfer amount exceeds allowance"
    );
  });
  ...
  it("events: should fire Transfer event properly", async () => {
    ...
    await expectEvent(res, "Transfer", {
      from: admin,
      to: alice,
      value: "2666",
    });
  ...
  it("events: should fire Approval event properly", async () => {
    ...
    await expectEvent(res, "Approval", {
      owner: admin,
      spender: alice,
      value: "2666",
    });
  ...
  it("onlyOwner: should change router", async function () {
    await expectRevert(
      instance.changeRouter(bob, { from: alice }),
  ...
  it("onlyOwner: should exclude from reward", async function () {
    await expectRevert(
      instance.excludeFromReward(bob, { from: alice }),
      "Ownable: caller is not the owner"
    );
  ...
  it("onlyOwner: should include in reward", async function () {
    await expectRevert(
      instance.includeInReward(bob, { from: alice }),
      "Ownable: caller is not the owner"
    );

    await expectRevert(
      instance.includeInReward(bob, { from: admin }),
  ...
  it("onlyOwner: should exclude from fee", async function () {
    await expectRevert(
      instance.excludeFromFee(bob, { from: alice }),
      "Ownable: caller is not the owner"
    );
  ...
  it("onlyOwner: should include in fee", async function () {
    await expectRevert(
      instance.includeInFee(bob, { from: alice }),
      "Ownable: caller is not the owner"
    );
  ...
  it("anti dump: should spend <= 30% in a day", async function () {
    ...
    await expectEvent(res, "Transfer", { from: alice, to: bob, value: "30" });
  ...
  it("max: onlyOwner [fail]", async function () {
    await expectRevert(
      instance.updateMaxPerc("1000", { from: alice }),
      "Ownable: caller is not the owner"
    );
  });
  
  it("max: onlyOwner [succeed]", async function () {
    await instance.updateMaxPerc("1000", { from: admin })
  });
  
  it("max: greater than precision [fail]", async function () {
    await expectRevert(instance.updateMaxPerc("100000", { from: admin }), "Max transaction percentage must be less than precision");
  });
  
  it("max: <= precision [succeed]", async function () {
    await instance.updateMaxPerc("10000", { from: admin });
  });
});
